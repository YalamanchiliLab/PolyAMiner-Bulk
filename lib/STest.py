...
def runBBtest(apa_matrix, nc, nt, out, key, npc,logfile):
	try:
		...
		gene_df=pd.DataFrame(result, columns = ["Gene","No.PAsites","PolyASites","DeltaU","MaxPASite","MaxDeltaU","G_Pval"])
		gene_df['AdjG_Pval'] = (multi.multipletests(gene_df['G_Pval'], method='fdr_bh', is_sorted=False, returnsorted=False))[1].tolist()		
...
def iNMFtest(outDir, fkey, nc, nt, ni, npc, matrix,logfile):	
	try:
		...
		gene_df=pd.DataFrame(result, columns = ["Gene","No.PAsites","PolyASites","APAscores","MaxAPAsite","MaxAPAscore","Stability","LRstat","G_Pval"])
		gene_df['G_Pval'] = gene_df['G_Pval'].astype(float)
		gene_df['AdjG_Pval'] = (multi.multipletests(gene_df['G_Pval'], method='fdr_bh', is_sorted=False, returnsorted=False))[1].tolist()		
