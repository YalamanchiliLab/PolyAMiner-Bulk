...
def MakeMatrix(outDir, npc, fkey, PA_P, PA_A, M, controls, treated, mip, mge, mode,samples,logfile,pa_usage):
	...
	if pa_usage =="all":
		os.system("cp "+outDir + fkey + '_APA.CountMatrix.txt '+outDir + fkey + '_PAusage.CountMatrix.txt')
	if pa_usage =="filtered":
		os.system("cp "+outDir + fkey + '_APA.CountMatrix.GFil.PA.PR.txt '+outDir + fkey + '_PAusage.CountMatrix.txt')
