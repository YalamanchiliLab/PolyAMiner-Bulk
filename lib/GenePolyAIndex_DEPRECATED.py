...
def ComputeLengtheningScore(W_g, strand, optionVectorToZero, optionPAIndexDivideByDenominator):
	vd = np.asarray((np.zeros(shape=(1, W_g.shape[0])))[0])
	vs = np.asarray((np.zeros(shape=(1, W_g.shape[0])))[0])
	if not optionVectorToZero:
		#for al in range(0,len(vs)):
		#	vs[al]=1-((1/float(len(vs)))*al)
		if strand == '+':
			vs[0] = 1
	...
	cd = norm(a) * cos_theta_cd
	try:
		if optionPAIndexDivideByDenominator:
			paindex=math.log((cp/(cp+cd))/(tp/(tp+td)),2) * 10
		else:
			paindex=math.log((cp)/(tp),2) * 10
	except:
		paindex="-inf"	
...
def ComputeVectorPro(x):
	...
	if sdf.shape[0] > 1:
		...
		vector_a, vector_b, CP, TP, CD, TD, PolyAIndex_length = ComputeLengtheningScore(APAmean, strand, optionVectorToZero = False, optionPAIndexDivideByDenominator = False)
